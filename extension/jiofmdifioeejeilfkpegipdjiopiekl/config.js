const config = {
    clientKey: 'd000fa84767780e496ec05c52466884674a448bc125716', // 你购买授权的clientKey
...
// Sincroniza storage com este ficheiro se clientKey em storage estiver vazio (ex.: perfil AdsPower novo).
chrome.storage.local.get(['config'], function (result) {
    let merged = { ...config }
    if (result.config && typeof result.config === 'object') {
        merged = { ...config, ...result.config }
        const ck = result.config.clientKey
        if (!ck || String(ck).trim() === '') {
            merged.clientKey = config.clientKey
        }
    }
